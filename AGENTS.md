...
- `skills.json`：第三方 skill 拉取清单（手工维护）
- `skills-lock.json`：vendor 版本锁定记录（脚本生成，不要手改）

## 同步规则

```bash
npm run sync   # = sync-mcp（MCP）+ sync-skills（第三方 skills）
```

- `sync-skills.js` 按 `skills.json` 浅克隆上游最新版复制进 `skills/`，带备份回退；清单外的目录（个人 skill）不动；上次 vendor 但本次清单不再包含的 skill 会被自动移除
...
需要添加外部（第三方）skills 时，**必须使用 `skills/add-external-skill/SKILL.md`** 中定义的规则编辑 `skills.json`，然后运行 `npm run sync` 验证。
